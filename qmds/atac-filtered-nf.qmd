...
The multiqc html (separate files), summarises the alignment statistics along with the summary of Fraction of Reads in Peak (FRIP's) generated via <a href="https://bowtie-bio.sourceforge.net/bowtie2/index.shtml"> `bowtie2` </a> and <a href="https://subread.sourceforge.net/featureCounts.html"> `featureCounts` </a>
...
p1 <- list.files("./figures", pattern = "cluster", full.names = T)
...
p2 <- list.files("./figures", pattern = "PCA", full.names = T)
...
p3 <- list.files("./figures", pattern = "MAPlot", full.names = T)
